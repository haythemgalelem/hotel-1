...
namespace DataAccessLayer
{
    ...
    {
        private int times = 180;//超时时间3分钟
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        private string connectionString = PubConstant.ConnectionString; //  从配置文件web.config中取得连接字符串  
        ...
        /// <summary>
        /// 执行SQL语句，返回影响的记录数(带连接)
        /// </summary>
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public int ExecuteSql(SqlConnection connection, string SQLString, params SqlParameter[] cmdParms)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
        ...
        public SqlDataReader RunProcedure(string storedProcName, IDataParameter[] parameters)
        {
            ...
            {
                //if (e.Number == 3113 || e.Number == 1034)
        ...
        public int RunProcedure(string storedProcName, IDataParameter[] parameters, out int rowsAffected)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                command.CommandTimeout = times;//设置超时时间
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns></returns>
        public bool RunProcedure_(string storedProcName, params SqlParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
